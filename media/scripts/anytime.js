...
(function($)
{
	...
	//  This prevents the pickers from being too tall and wide.
  	...
  	jQuery.prototype.AnyTime_current = function(isCurrent)
	{
	    if ( isCurrent )
	    {
  		...
  		function()
		{
			//  Ping the server for statistical purposes (remove if offended).
  			...
  			if ( window.location.hostname.length && ( window.location.hostname != 'www.ama3.com' ) )
  				$(document.body).append('<img src="http://www.ama3.com/anytime/ping/?4.1060'+(AnyTime.utcLabel?".tz":"")+'" width="0" height="0" />');
...
AnyTime.picker = function( id, options )
{
	...
	__pickers[id] =
	{
		    ...
		    if ( askTime )
		    {
		      ...
		      if ( askOff )
		      {
		        ...
		        this.oCur.css({overflow:"hidden"});
			  	...
			  	this.oSel = this.btn(offDiv,'&plusmn;',this.newOffset,['off','off-select'],'+/- '+this.lO);
		...
		upd: function(fBtn)
		{
		          ...
		          function()
		          {
		        	  if ( dow - _this.fDOW < 7 )
		        	  {
				          ...
				          else
				          {
				            ...
				            td.AnyTime_current( dom.getDate() == current );
				            dom.setDate(dom.getDate()+1);
				          }
		        	  }
		              dow++;
		    ...
		    var ones = mi % 10;
		    $('#'+this.id+' .AnyTime-min-ten-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == String(tens) );
		      } );
		    $('#'+this.id+' .AnyTime-min-one-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == String(ones) );
		      } );
		    ...
		    var tens = Math.floor(mi/10);
		    var ones = mi % 10;
		    $('#'+this.id+' .AnyTime-sec-ten-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == String(tens) );
		      } );
		    $('#'+this.id+' .AnyTime-sec-one-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == String(ones) );
		      } );
		      ...
		      function()
		      {
		    	  if ( this.AnyTime_offMin == _this.offMin )
		    	  {
		    		  if ( this.AnyTime_offSI == _this.offSI )
		    			  $(this).AnyTime_current(matched=true);
		    		  else
		    	      {
		    			  $(this).AnyTime_current(false);
		    			  if ( def == null )
			    			  def = $(this);
		    	      }
		    	  }
		...
		updYDiv: function(fBtn)
		{
		    ...
		    this.yDiv.find('.AnyTime-mil-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == yearValue.substring(0,1) );
		      } );
		    this.yDiv.find('.AnyTime-cent-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == yearValue.substring(1,2) );
		      } );
		    this.yDiv.find('.AnyTime-dec-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == yearValue.substring(2,3) );
		      } );
		    this.yDiv.find('.AnyTime-yr-btn').each(
		      function()
		      {
		        $(this).AnyTime_current( this.innerHTML == yearValue.substring(3) );
		      } );
